...
class YTSStore:
    ...
    def submit_review(self, collab_id, video_url):
        ...
        # 提醒：钉钉群即时通知审核同学（失败静默，不影响主流程）
        name = (self._get(collab_id) or {}).get("channel_name") or collab_id
        N.notify_review_submitted(name, video_url)
...
def get_yts_store():
    ...
    注意 @st.cache_resource：Streamlit 每次交互都会重跑整个脚本，
    不加这个装饰器的话，store 和它内部的缓存每次点击都会被重建，
    导致 5 分钟缓存完全失效、每个页面都全量重拉宜搭（卡顿根因）。
    加上后 store 在整个应用生命周期内只建一次，缓存真正生效。
    """
    cfg = _cfg()
