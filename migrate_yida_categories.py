# -*- coding: utf-8 -*-
"""
⚠️ 已废弃（2026-08-24）：请改用 backfill_dual_categories.py
--------------------------------------------------------------------
本脚本是双垂类改造前的旧口径：把「带货垂类」写进原「垂类」下拉。
2026-08-24 起原「垂类」下拉已改存「内容垂类」，再跑本脚本会用带货垂类
把内容垂类盖错。双垂类的全量回填/对齐请用 backfill_dual_categories.py。
...
def main():
    if "--force-deprecated" not in sys.argv:
        print("⚠️ 本脚本已废弃（2026-08-24 双垂类改造后口径已变）。\n"
              "请改用：python3 backfill_dual_categories.py\n"
              "如确需运行旧逻辑，加参数 --force-deprecated。")
        return
